...
class TravelGuideDecisionLogic:
    ...
    def process_user_message(self, message: str, user_id: str) -> Dict[str, Any]:
        try:
            ...
            # NEU: Nur scrapen, wenn Stadt UND Hotel/Unterkunft im Text
            message_lower = message.lower()
            has_city = False
            has_hotel_word = any(word in message_lower for word in ["hotel", "unterkunft"])
            has_weather_word = any(word in message_lower for word in ["wetter", "klima"])
            # Prüfe, ob eine Stadt erkannt wurde (über entities)
            if 'hotel_location' in entities and entities['hotel_location']:
                has_city = True
            elif 'destination' in entities and entities['destination']:
                has_city = True
            elif self._extract_location_from_message(message):
                has_city = True

            # Wetterlogik: Stadt + Wetter/Klima => Wetter-API
            if has_city and has_weather_word:
                # Extrahiere Stadt
                if 'hotel_location' in entities and entities['hotel_location']:
                    city = entities['hotel_location']
                elif 'destination' in entities and entities['destination']:
                    city = entities['destination']
                else:
                    city = self._extract_location_from_message(message)
                if city:
                    return self._handle_weather_request(user_id, {'weather_location': city}, message)
            ...
            if has_city and has_hotel_word:
                ...
                # Nur Stadt, aber KEIN Hotel/Unterkunft: KI antwortet
                try:
                    ...
                    logger.error(f"Fehler bei KI-Antwort: {e}")
                    return {
                        'type': 'error',
            ...
            # Standard-Intent-Logik wie gehabt
            if intent == 'greet':
                ...
                # KEINE automatische Hotelsuche mehr bei nur einer Stadt!
    ...
    def _handle_destination_provided(self, message: str, user_id: str, entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if destination:
            ...
            # KEINE automatische Hotelsuche mehr! Nur Info speichern.
            return {
                'type': 'destination_saved',
                'message': f'Reiseziel "{destination}" wurde gespeichert. Sie können nun nach Hotels, Flügen oder Wetter fragen.',
                'suggestions': [
                    f'Hotels in {destination} finden',
                    f'Wie ist das Wetter in {destination}?',
...
"""
    ...
    def _handle_general_question(self, message: str, user_id: str) -> Dict[str, Any]:
        ...
        # Hotelsuche wird jetzt nur noch in process_user_message ausgelöst!

        # Fallback: KI-Antwort von Ollama
        try:
            ollama_response = ai_service.generate(message)
            return {
                'type': 'general',
